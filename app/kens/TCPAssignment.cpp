...
void TCPAssignment::syscall_getsockname(UUID syscallUUID, int pid, int param1,
  ...
  pid_fd pf1=make_pair(pid,param1);
  address_port ap1;
  if ( (src_m.find(pf1) == src_m.end()) && (cli_m.find(pf1) == cli_m.end()) ) {
     return returnSystemCall(syscallUUID, -1); 
  }
  else if ( (src_m.find(pf1) != src_m.end()) && (cli_m.find(pf1) == cli_m.end()) ) {
     ap1 = src_m[pf1];
  }
  else if( (src_m.find(pf1) == src_m.end()) && (cli_m.find(pf1) != cli_m.end()) ) {
    Four_tuple ssdd = cli_m[pf1];
    ap1 = make_pair(get<0>(ssdd),get<1>(ssdd));
  }
  else{
    ap1 = src_m[pf1];
    //perror("didn't considerf\n");
  }
  struct sockaddr_in* socksock = (sockaddr_in*) param2_ptr;
  //memset(&socksock, 0, sizeof(socksock));
  socksock->sin_family = AF_INET;
...
void TCPAssignment::syscall_connect(UUID syscallUUID, int pid, int param1, 
  ...
  uint16_t checksum = NetworkUtil::tcp_sum(src_ip_32,dest_ip_32,temp,20); //
  checksum = ~checksum;
  ...
  pid_fd pf1 = make_pair(pid, param1);
  address_port ap1 = make_pair(dest_ip_32,dest_port);


  Four_tuple ssdd= make_tuple(src_ip_32, source_port, dest_ip_32, dest_port);
  cli_m.insert(pair<pid_fd, Four_tuple>(pf1, ssdd));
  //return;
  ...
  switch(real_flag){
    case 0b00000010:{ //connect, syn
        ...
        if(for_listen[server_address_port]==accepted_que[server_address_port].first.size()){
          if(for_listen[server_address_port]==0){
            break;
          }
          else{
